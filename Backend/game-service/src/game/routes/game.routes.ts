...
export async function registerGameRoutes(fastify: FastifyInstance) {
   ...
   */
  fastify.get<{ Params: { userId: string } }>('/game/stats/:userId', async (request, reply) => {
    return httpController.getUserStats(request, reply);
  });

  fastify.get<{ Params: { userId: string }; Querystring: { limit?: string } }>('/game/recent-games/:userId', async (request, reply) => {
    return httpController.getUserRecentGames(request, reply);
  });

  fastify.log.info('Game WebSocket route registered: /game/ws');
  fastify.log.info('Game HTTP routes registered: /game/stats/:userId, /game/recent-games/:userId');
}
