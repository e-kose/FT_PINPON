	...
	protected async handleSubmit(e: Event): Promise<void> {
		...
		try {
			...
			.then(data => {
				...
				if (data.succes)
					fillIndex(succesForm.userRegister("Onaylandı", "Kayıt işleminiz başarıyla tamamlandı ve işleme alındı."));
				else
					fillIndex(succesForm.userRegister("Başarısız", data.message));

				fillIndex("<LoginForm> </LoginForm>");
			})
			.catch(error => {
          ...
          <div class="p-8 space-y-6 md:space-y-8 sm:p-10">
              ...
              <form class="space-y-6 md:space-y-8" action="#">
                  ...
                  <p class="text-sm font-light text-gray-500 dark:text-gray-400">
                      Zaten hesabınız var mı? <a href="#" class="font-medium text-white bg-blue-900 hover:bg-blue-800 px-2 py-1 rounded hover:underline dark:bg-blue-900 dark:hover:bg-blue-800">Buradan Giriş Yapın</a>
                  </p>
              </form>
          </div>
      </div>
