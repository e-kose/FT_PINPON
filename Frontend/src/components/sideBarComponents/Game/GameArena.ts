...
export class GameArena extends LocalizedComponent {
	...
	protected renderComponent(): void {
		this.innerHTML = `
            <div class="flex flex-col items-center justify-center w-full h-full relative">
                ...
                <div id="playerHUD" class="w-[800px] flex justify-between items-end mb-2 px-4 shadow-xl">
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div class="flex flex-col">
                             <span class="text-xs text-cyan-400 font-bold tracking-widest mb-1">PLAYER 1</span>
                             <span id="nameLeft" class="text-2xl text-white font-black tracking-wider text-shadow font-orbitron">PLAYER 1</span>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-4 flex-row-reverse text-right">
                        ...
                        <div class="flex flex-col">
                             <span class="text-xs text-fuchsia-400 font-bold tracking-widest mb-1">PLAYER 2</span>
                             <span id="nameRight" class="text-2xl text-white font-black tracking-wider text-shadow font-orbitron">PLAYER 2</span>
                        </div>
                    </div>
                </div>

                <div class="relative">
                    <!-- Score Board (Floating inside) -->
                    ...
                    <div id="gameOverlay" class="absolute inset-0 bg-black/85 backdrop-blur-sm flex flex-col items-center justify-center opacity-0 pointer-events-none transition-opacity duration-300 z-20">
                        <div id="overlayTitle" class="text-6xl font-black text-white mb-4 text-shadow-glow font-orbitron">GAME OVER</div>
                ...
                <div class="mt-4 text-white/40 text-sm tracking-widest">CONTROLS: W/S or ARROW UP/DOWN</div>
            </div>
        `;
	}
	...
	public renderGame(state: any): void {
		if (!this.ctx) return;
		const ctx = this.ctx;
		...
		if (state.players?.left) {
			if (scoreLeft && state.players.left.score !== undefined) scoreLeft.textContent = state.players.left.score;
			if (!this.isLocal && nameLeft && state.players.left.id) nameLeft.textContent = state.players.left.id.substring(0, 10).toUpperCase();
		}

		if (state.players?.right) {
			if (scoreRight && state.players.right.score !== undefined) scoreRight.textContent = state.players.right.score;
			if (!this.isLocal && nameRight && state.players.right.id) nameRight.textContent = state.players.right.id.substring(0, 10).toUpperCase();
		}
	...
	public showOverlay(payload: any): void {
		...
		if (title) {
			title.textContent = payload.title || "GAME OVER";
			title.className = "text-6xl font-black mb-4 text-shadow-glow font-orbitron";
			if (payload.title === "VICTORY") {
				title.classList.add("text-green-500");
			} else if (payload.title === "DEFEAT") {
