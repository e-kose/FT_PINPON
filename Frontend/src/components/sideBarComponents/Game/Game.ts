...
export class Game extends LocalizedComponent {
	...
	private setupEvents(): void {
		...
		// Global Back Button (if we had one, logic was previously inside blocks, but we can have it global if needed)
		// For now, the sub-components handle their own 'back' or 'cancel' flows except for the global 'Game Over' continue.
		// If we need a global back button, we can add it here.
	...
	private connect(): void {
		...
		gameWebSocketService.addListener({
			onOpen: () => {
				...
				this.sendMessage('SET_USER_ID', { userId: String(user.id) });
			},
			onClose: () => {
	...
	private handleMessage(msg: any): void {
		...
		switch (msg.type) {
			case 'MATCHMAKING_SEARCHING':
				...
				this.updateQueueStatus("FINDING OPPONENT...", msg.payload);
				break;
			case 'TOURNAMENT_QUEUE_JOINED':
				this.switchScreen('queue');
				this.switchScreen('queue');
				this.updateQueueStatus("WAITING FOR TOURNAMENT...", null);
				break;
				break;
			...
			case 'TOURNAMENT_STATE':
				...
				if (this.currentScreen !== 'game' || msg.payload.tournament.state === 'finished') {
					...
					const t = msg.payload.tournament;
					if (t.state === 'finished' && t.bracket.winnerId) {
						const winnerId = t.bracket.winnerId;
						const myId = String(getUser()?.id);
						const bracket = this.querySelector('game-bracket') as Bracket;
						if (winnerId === myId) {
							bracket?.updateStatus("TOURNAMENT CHAMPION! 🏆", true, 'return');
						} else {
							bracket?.updateStatus(`Tournament Won by ${winnerId.substring(0, 8)}`, true, 'return');
						}
					}
				}
				break;
			...
			case 'STATE_UPDATE':
				...
				break;
			case 'GAME_OVER':
				// Handle specific flow based on mode
				if (this.gameMode === 'tournament') {
					this.handleTournamentGameOver(msg.payload);
				} else if (this.gameMode === 'local') {
					// Show Score directly
					let subtitle = "GAME FINISHED";
					if (this.lastGameState && this.lastGameState.players) {
						const leftScore = this.lastGameState.players.left?.score || 0;
						const rightScore = this.lastGameState.players.right?.score || 0;
						subtitle = `${leftScore} - ${rightScore}`;
					}
					this.showOverlay({ ...msg.payload, title: "GAME OVER", message: subtitle });
				} else if (this.gameMode === 'online') {
					const myId = String(getUser()?.id);
					const isWinner = msg.payload.winnerId === myId;
					const title = isWinner ? "VICTORY" : "DEFEAT";
					this.showOverlay({ ...msg.payload, title });
				}
				break;
			case 'ERROR':
	...
	private handleTournamentGameOver(data: any): void {
		const myId = String(getUser()?.id);
		...
		if (isWinner) {
			bracket?.updateStatus("VICTORY! Waiting for next round...", false);
		} else {
			bracket?.updateStatus(`ELIMINATED by ${data.winnerId.substring(0, 8)}`, true, 'leave');
		}
	}
	...
	private renderGame(state: any): void {
