...
export class Bracket extends LocalizedComponent {
      ...
      <div class="flex flex-col items-center justify-start w-full h-full p-8 pt-24 space-y-8">
        ...
        <div id="bracketHeader" class="text-center space-y-4 z-10">
          <h2 class="text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-600 tracking-[0.2em] uppercase filter drop-shadow-[0_0_15px_rgba(0,243,255,0.3)]">
            ${t("tournament_bracket_title") || "TOURNAMENT"}
          </h2>
	...
	public updateStatus(message: string, showAction: boolean = false, actionType: 'leave' | 'return' = 'leave'): void {
		// Prevent overwriting the final cleanup state with intermediate messages
		...
		if (statusEl) {
			...
			if (message.includes("Victory") || message.includes("CHAMPION")) statusEl.className += " text-cyan-400 drop-shadow-[0_0_10px_rgba(34,211,238,0.5)]";
			else if (message.includes("Eliminated")) statusEl.className += " text-red-500 drop-shadow-[0_0_10px_rgba(239,68,68,0.5)]";
		...
		if (actionsEl) {
			if (showAction) {
				...
				if (actionType === 'return') {
					...
					btn.innerHTML = `<span class="relative z-10">Leave Tournament</span><div class="absolute inset-0 bg-red-500/20 translate-y-full group-hover:translate-y-0 transition-transform duration-300"></div>`;
	...
	public renderBracket(tournament: any): void {
		...
		rounds.forEach((round: any, rIndex: number) => {
			...
			round.matches.forEach((match: any) => {
				...
				if (match.status === 'in_progress') {
					...
					statusIndicator = `<div class="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-0.5 bg-black border border-cyan-500 text-cyan-400 text-[10px] font-bold tracking-widest uppercase shadow-[0_0_10px_rgba(6,182,212,0.3)] z-20">LIVE</div>`;
				} else if (match.status === 'scheduled') {
					borderColor = "border-yellow-500/30";
					statusIndicator = `<div class="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-0.5 bg-black border border-yellow-500/50 text-yellow-500/80 text-[10px] font-bold tracking-widest uppercase z-20">UP NEXT</div>`;
				...
				// Highlight User
				const isUserMatch = match.player1Id == currentUserId || match.player2Id == currentUserId;
				if (isUserMatch) {
				...
				const p1Name = this.formatPlayerName(match.player1Id);
				const p2Name = this.formatPlayerName(match.player2Id);
				...
				// Helper to generate player row HTML
				const getPlayerRow = (name: string, isWinner: boolean, id: string | null) => {
					...
					return `
						<div class="${rowBase}">
							...
							${isWinner ? `<span class="text-[10px] font-black text-black bg-cyan-400 px-1.5 py-0.5 z-10 shadow-[0_0_10px_rgba(34,211,238,0.5)] shrink-0 ml-2">WIN</span>` : ''}
						</div>
					`;
				};
	...
	private formatPlayerName(id: string | null): string {
		if (!id) return "---";
	...
	private leaveTournament(): void {
		if (confirm("Are you sure you want to leave the tournament?")) {
			...
			// The Game component will handle the screen switch on response
		}
	}
