		...
		this.innerHTML = `
			<div class="space-y-4 sm:space-y-6 md:space-y-8">
				<div class="flex items-center space-x-2 sm:space-x-3">
					<div class="w-8 h-8 sm:w-10 sm:h-10 bg-gradient-to-r from-emerald-500 to-green-600 rounded-lg flex items-center justify-center">
					...
					<h2 class="text-lg sm:text-xl md:text-2xl font-bold text-gray-900 dark:text-white">${t("account_settings_title")}</h2>
				</div>
				...
				<div class="bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-700/50 dark:to-gray-800/50 p-3 sm:p-4 md:p-6 rounded-lg sm:rounded-xl border border-gray-200/50 dark:border-gray-600/50">
					<h3 class="text-sm sm:text-base md:text-lg font-semibold text-gray-900 dark:text-white mb-3 sm:mb-4">${t("account_settings_info_title")}</h3>
					<div class="space-y-2 sm:space-y-3 md:space-y-4">
						...
						<div class="flex flex-col sm:flex-row sm:justify-between sm:items-center py-2 sm:py-3 border-b border-gray-200 dark:border-gray-600 gap-1 sm:gap-0">
							...
							<span class="text-xs sm:text-sm font-bold text-gray-900 dark:text-white">${new Date(user.created_at).toLocaleDateString("tr-TR")}</span>
						</div>
						<div class="flex flex-col sm:flex-row sm:justify-between sm:items-center py-2 sm:py-3 border-b border-gray-200 dark:border-gray-600 gap-1 sm:gap-0">
							<span class="text-xs sm:text-sm font-medium text-gray-600 dark:text-gray-400">${t("account_settings_field_updated")}</span>
							<span class="text-xs sm:text-sm font-bold text-gray-900 dark:text-white">${new Date(user.updated_at).toLocaleDateString("tr-TR")}</span>
						</div>
					</div>
				</div>

				<div class="bg-gradient-to-br from-red-50 to-pink-50 dark:from-red-900/20 dark:to-pink-900/20 p-3 sm:p-4 md:p-6 rounded-lg sm:rounded-xl border-2 border-red-200 dark:border-red-700">
				...
				<div class="delete-modal hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-2 sm:p-4">
					<div class="bg-white dark:bg-gray-800 rounded-lg sm:rounded-xl shadow-2xl p-4 sm:p-6 md:p-8 max-w-md w-full mx-2 sm:mx-4">
						<div class="text-center">
							<div class="w-12 h-12 sm:w-16 sm:h-16 mx-auto mb-3 sm:mb-4 bg-red-100 dark:bg-red-900/20 rounded-full flex items-center justify-center">
				...
				<div class="account-message-container mt-4 sm:mt-6"></div>
			</div>
		`;
